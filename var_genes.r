...
meanCVfit = function(count.data, reads.use=FALSE, do.text=FALSE, diffCV.cutoff=0.5, do.spike=FALSE, main.use=NULL, prefix=NULL){
    ...
    # Plot
    if(!is.null(prefix)){
        pdf(paste(prefix, '.var_genes.pdf', sep=''), width=10, height=10)
        par(mfrow=c(2,2))
    }
