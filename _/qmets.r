...
colnames(data) = c('id', 'folder', 'pattern', 'path')
...
for(i in 1:nrow(data)){
    ...
    print(paste('Found', length(files), '*ReadQualityMetrics.txt files'))
    ...
    files = list.files(path=path, pattern=pattern, full.names=TRUE)
    print(paste('Found', length(files), '*HUMAN.*summary.txt files'))
    ...
    files = list.files(path=path, pattern=pattern, full.names=TRUE)
    print(paste('Found', length(files), '*MOUSE.*summary.txt  files'))
    new_ids = gsub('.*/', '', gsub('_star.*', '', files))
    ...
    files = list.files(path=path, pattern=pattern, full.names=TRUE)
    print(paste('Found', length(files), '*categorized_cellTypes.txt files'))
    new_ids = gsub('.*/', '', gsub('_star.*', '', files))
...
# Print cells per collection
cells.500 = list()
cells.1000 = list()
for(i in 2:nrow(t.data)){
    group1 = as.character(t.data[i,'group_ids']); group2 = as.character(t.data[i,'group_ids']);
    genes1 = t.data[i,'num_genes']; genes2 = t.data[i,'num_genes'];
    cells1 = t.data[i,'num_cells']; cells2 = t.data[i,'num_cells'];
    if(is.na(cells2)){next}
    if(genes2 == 500){cells.500[[group2]] = cells2}
    if(genes2 == 1000){cells.1000[[group2]] = cells2}
    if(group1 == group2 & ((genes1 < 500) != (genes2 < 500))){cells.500[[group2]] = cells2}
    if(group1 == group2 & ((genes1 < 1000) != (genes2 < 1000))){cells.1000[[group2]] = cells2}
}
print(cells.500)
print(cells.1000)
...
barplot(t(as.matrix(doublet_rates)), col='orange', ylab='Estimated doublet rate', las=2)
dev.off()
